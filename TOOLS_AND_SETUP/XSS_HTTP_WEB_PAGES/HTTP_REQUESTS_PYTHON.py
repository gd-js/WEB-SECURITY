...
# browser connects via string input dns->ip to server with tcp/ip (and ssl/tls for https to sign client and server)
# browser sends https request over tls/tcp to server ip (request line(verb, path, version\r\n)headers(name(user-agent, host, accept, X-): value)\r\n)
