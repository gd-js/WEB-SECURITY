...
ssl handshake
```
C protocol version choices, cipher suite choices, secured random number
S protocol version selection, cipher suite selection, encryption algo selection, show certificate
C pre-key = public certificate + secured random number
S key = secured random number + another secured random number
